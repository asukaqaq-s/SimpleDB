...
TEST(ExpressionTest, ColumnTest) {
    ...
    auto column_value_expression1 = new ColumnValueExpression(TypeID::INTEGER, 0, "colA", &schema);
    auto column_value_expression2 = new ColumnValueExpression(TypeID::INTEGER, 1, "colB", &schema);
...
TEST(ExpressionTest, ConjunctionExpressionTest) {
    auto true1 = std::make_unique<ConstantValueExpression>(Value(true));
    auto false1 = std::make_unique<ConstantValueExpression>(Value(false));
    auto true2 = std::make_unique<ConstantValueExpression>(Value(true));
    auto false2 = std::make_unique<ConstantValueExpression>(Value(false));

    auto conj_and_false = std::make_unique<ConjunctionExpression>(ExpressionType::ConjunctionExpression_AND, true1.get(), false1.get());
    auto conj_or_true = std::make_unique<ConjunctionExpression>(ExpressionType::ConjunctionExpression_OR, true1.get(), false1.get());
    auto conj_and_true = std::make_unique<ConjunctionExpression>(ExpressionType::ConjunctionExpression_AND, true1.get(), true2.get());
    auto conj_or_false = std::make_unique<ConjunctionExpression>(ExpressionType::ConjunctionExpression_OR, false1.get(), false2.get());
