...
TEST(UpdateParseTest, BasicTest) {
    ...
    try
    {
        ...
        std::string s = "create table test1 if not exist "; 
        str << s;
        str << "( cola varchar(20), colb int, colc decimal, cold char(20))";

        
        Parser p(str.str(), txn.get(), mdm.get());
        auto stmt = (p.ParseCreate());
        auto create_table = static_cast<CreateTableStmt*>(stmt.get());

        std::cout << create_table->sch_->ToString() << std::endl;
...
TEST(CreateIndexParseTest, BasicTest) {
     char buf[100];
    std::string local_path = getcwd(buf, 100);
    std::string test_dir = local_path + "/" + "test_dir";
    std::string test_file = "test1";
    ...
    try
    {
        ...
        std::string s = "create index qaq on qaq12 cola";
        str << s;

        
        Parser p(str.str(), txn.get(), mdm.get());
        auto stmt = (p.ParseCreate());
        auto create_table = static_cast<CreateIndexStmt*>(stmt.get());

        std::cout << create_table->ToString() << std::endl;
