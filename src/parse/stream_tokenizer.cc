...
StreamTokenizer::StreamTokenizer(std::istream &is)
    ...
    // whitespace, should not be seen, '\0' and ' '
    WhiteSpaceChars(0x00, 0x20);
    
    // word char: alphabet and other characters
    WordChars('A', 'Z');
    WordChars('a', 'z');
    WordChars(0xA0, 0xFF);

    // only support //, not support /**/
    CommentChar('/');
    // CommentChar('#');
    // CommentChar('-');
...
// |        PUBLIC METHOD       | 
// ------------------------------
void StreamTokenizer::CommentChar(int index) {
...
int StreamTokenizer::NextToken() {
    ...
    // 3. eol and line feed, may be not skip it and view it as a token
    // 4. comment, skip it
    ...
    /*******************************
     * phase 1: parse white space
     *******************************/

    // skip any whitespace. deal with eol along the way
    while ((ch = reader_.Read()) && IsWhiteSpace(ch)) {
        // just skip and do nothing, pos will increase in Read method
    }
    

    // \r\n is also viewed as a newline sign, skip \n if in combination with \r
    // if not, we should undo read operations by Unread.
    // \r not followed by \n, there may be other meanings that cannot be 
    // skipped as a whitespace.
    if (ch == '\r' && ((ch == reader_.Read()) && ch != '\n')) {
        
        // if the most recently read ch is not EOF, Undo it
        if (ch != TT_EOF) {
            reader_.Unread(ch);
        }
    ...
    /**************************
     * phase 2: parse comment
     **************************/
    if (ch == '/') {
        ch = reader_.Read();
        
        // comments '//' 
        // read until newline, carraige return(回车), eof
        if ((ch == '/') && SlashSlash_) {
            ...
            // when the above loop jumps out, the first character of the next line 
            // has been read. unread if not end of file
            if (ch != TT_EOF) {
                reader_.Unread(ch);
            }

            // since current token is newline , carraige return or eof, 
            // we should not view them as a token, so we should return the next token
            return NextToken();
        }
    ...
    // such as "" or ''
        ...
        while ((ch = reader_.Read()) &&  // note taht read must be on the front
               (ch != TType_) && // the next quote, " or '
               (ch != '\n') && (ch != '\r') &&  // eol
               (ch != TT_EOF)) /* eof */ {
            ...
            // if ' is not followed by '\', view ch as just a letter
            tok_buf.push_back(ch);    
        }

        // if read the first letter of the next line, unread to avoid error
        if (ch != TType_ && ch != TT_EOF) {
            reader_.Unread(ch);
        }
        SVal_ = tok_buf;
    } else {
        // ch has read when parse whitespace
        // delim, such as '-', ';', '+', '%', '*', '/', '(', ')' 
        TType_ = ch;
    }
